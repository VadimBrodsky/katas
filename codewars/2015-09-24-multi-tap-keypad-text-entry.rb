...
def presses(phrase)
  keypad = Keypad.new
  phrase.to_s.downcase.split('').map { |c| keypad.char_to_presses(c) }.inject(:+)
end
...
class Keypad
  ...
  def initialize
    @keypad = (1..9).map{|i| [i.to_s]}.push(['*'],[' ', '0'],['#'])
    letters = ('a'..'z').to_a
    @keypad[1..8].map! do |a| 
      per_key = (['7', '9'].include? a[0]) ? 4 : 3
      a.unshift(letters.shift(per_key)).flatten!
    end
    @keypad
  end

  def char_to_presses(char)
    @keypad.map{|i| (i.include? char) ? i.index(char) + 1 : 0}.inject(:+)
  end
end

p presses('HOW U R')
p presses('0 0')
p presses('WHERE DO U WANT 2 MEET L8R')
...
class TestKata < MiniTest::Unit::TestCase
  def test_output
    ...
    assert_equal presses('WHERE DO U WANT 2 MEET L8R'), 47
  end
end
